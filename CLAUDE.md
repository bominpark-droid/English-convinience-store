...
## 무엇인가
초등학생용 영단어 학습 게임 웹앱. 단어를 외워 "영머니"를 벌고,
앱 안 편의점에서 실제 상품을 골라 부모가 실제로 사 주는 구조.

**여러 집이 같이 쓴다** (2026-08-28). 계정(아이디)을 만들면 그 계정마다
진행 상황·레벨·상점이 따로 간다. 지인 가족은 같은 주소로 들어와 가입만 하면 된다.
사용자: 박보민 대표의 초4 아들(국제학교 진학 예정), 지인 가족의 7세 유치원생.
...
- **index.html은 단일 파일**이다. HTML+CSS+JS가 한 파일 안에 있고, 빌드 과정도 번들러도 없다. 파일을 쪼개지 말 것.
  예외는 `sw.js`(오프라인 담당)·`manifest.json`·`icon-*.png` 넷뿐이다 — 브라우저가 **별도 파일이어야만**
  인정하는 것들이라 어쩔 수 없다. 앱 코드를 여기로 옮기지 말 것.
...
3. **단어 DB(`WORDS`) 구조를 깨지 말 것.** 레벨 **0~5**, 각 75개, 각 항목은 6필드 배열:
...
6. **레벨 범위는 `LV_MIN`(0) · `LV_MAX`(5) 상수로만** 돈다. `for(let l=1;l<=5;l++)` 같은
   숫자를 다시 박아 넣지 말 것. 레벨을 더 늘릴 때 여기저기 빠뜨리게 된다.

## 레벨 0 「왕초보 🐥」 — 7세·유치원용 (2026-08-28 신설)

한국 초등 1학년 필수 영단어 75개. 인사 10 · 숫자 10 · 색깔 8 · 가족 8 · 몸 7 ·
동물 10 · 음식 8 · 학교물건 8 · 기본동사형용사 6 순서로 **주제별로** 묶여 있다
(레벨 1~5는 알파벳 순. 왕초보는 하루 10개가 한 주제가 되도록 일부러 주제순으로 두었다).

**왕초보는 「말하기」 단계가 없다** (대표 지시 2026-09-03 — 7살 아이가 힘들어했다).
하루 흐름이 **단어 카드 → 예문 빈칸 → 퀴즈** 3단계다. 판단은 `skipSpeak()` 하나로 모았고
(`S.level === 0`), 말하기로 들어가는 길목인 `startSpeak()` 첫머리에서 되돌린다 —
예문 끝·홈 단추·`startQuiz` 가 전부 이 함수를 거치므로 여기만 막으면 된다.
`stageBar()` 도 왕초보면 3칸으로 그린다. **말하기 단계에는 영머니 보상이 없어
상점 물가는 그대로다.** 레벨 1로 올라가면 말하기가 다시 나온다.
(학교 스펠링은 별개 화면이라 영향 없다.)

- 시작 화면에 **[영어가 처음이에요 · 왕초보부터 시작할래요 🐥]** 단추(`startBeginner()`).
  레벨 테스트를 건너뛰고 바로 Lv.0 으로 시작하며 시작 보너스 300원은 그대로 준다.
- 레벨 테스트는 이제 **18문항**(6레벨 × 3문항). 레벨 1을 못 맞히면 Lv.0 으로 배정된다.
...
**말하기(녹음) 단계가 "사라졌다"는 신고의 정체 (2026-08-31):** 코드가 지워진 게 아니라,
마이크가 한 번 거부되면 `micState="no"` 가 되어 🎤 버튼 대신 자가 체크 모드만 보인다.
- `recStart` 실패 시 이제 **허용하는 방법 안내 모달**(아이폰: aA → 웹 사이트 설정 → 마이크 허용)을 띄운다.
- 자가 체크 화면과 설정의 마이크 줄에 **[🎤 마이크 다시 켜 보기]**(`retryMic`)가 있다 — 없애지 말 것.
- `stopAudio()` 는 예약된 발음 타이머(`ttsT/ttsRetryT`)도 취소한다. 안 그러면 녹음 중에
  뒤늦은 발음 재시도가 마이크에 끼어든다.

## 학교 스펠링 (2026-08-31 신설)

아들 학교의 시험 단어(예: "Room to Grow" 8단어)를 부모가 넣고, 아이가
**듣고 스펠링을 타자로 쓰는** 시험 연습을 하는 별도 섹션. 화면은 `scr-school`.
...
- **문제는 「영어 정의」, 답은 「단어 스펠링」이다** (대표 지시 2026-09-07).
  ...
  - **답을 자동으로 읽어주지 않는다.** 예전에는 화면에 들어오자마자 `speak(단어)` 를 불러
    소리만 듣고 받아쓰면 끝이었다. 되살리지 말 것.
  - **소리 힌트는 없다** (대표 지시 2026-09-07). 퀴즈(연습 「가리고 쓰기」·테스트)에서는
    발음을 들려주지 않는다 — 소리를 주면 뜻을 안 읽고 받아쓰기가 된다.
    막혔을 때 누르는 **한글 뜻 힌트**(`scHintKo`)만 남긴다.
    「단어 보기」와 연습의 「외우기」 칸은 배우는 단계이므로 🔊 를 그대로 둔다.
  - 영어 정의가 비어 있는 단어는 한글 뜻으로 출제하고 그 사실을 화면에 알린다.
- 시험 화면에는 **예문을 보여주지 않는다** (예문 안에 답 단어가 들어 있다).
- 입력은 `spellNorm`(소문자·공백 정리)으로 비교, `spellDiff` 가 글자별 초록/빨강 표시.
  입력칸은 자동 대문자·자동 수정을 꺼 둔다(`autocapitalize/autocorrect/spellcheck off`).
- 보상: 테스트 정답 단어당 +30(같은 단어는 하루 한 번만, 반복은 +5) · 만점 +100(세트당 하루 1회)
  · 연습 완주 +20(세트당 하루 1회). 기록은 `S.spellDay`(날짜 바뀌면 리셋).
- 부모 편집(`scEdit`, 부모 모드 전용): 세트마다 텍스트칸에 **한 줄 = 「단어 | 뜻 | 영어 정의 | 예문」**.
  빈 줄·단어 없는 줄은 걸러진다. 저장해야 반영.

## 오프라인 (2026-09-11 신설)

인터넷 없이도 앱이 열린다. `sw.js`(서비스 워커 — 화면 파일을 기기에 저장해 두는 작은 프로그램)가
index.html · quest.html · 구글 글꼴을 캐시한다.

- 방식은 **캐시 먼저 주고 뒤에서 새 판 받기**. 그래서 고친 내용은 **한 번 더 열어야** 보인다
  (대표에게 안내할 때 "새로고침 두 번"이라고 말할 것).
- **POST 와 script.google.com 은 캐시하지 않는다.** 저장·로그인은 언제나 실시간.
- `CACHE` 이름(`ecvs-v1`)을 바꾸면 옛 캐시를 싹 지운다. 캐시 구조를 바꿀 때만 올린다.
- **오프라인에서 한 공부가 날아가던 함정**: 예전 `Store.load()` 는 서버 응답이 오면 무조건 그걸 썼다.
  비행기 모드에서 공부 → 연결되면 서버의 옛 상태가 덮어썼다. 이제 `S.savedAt`(저장 시각)을 비교해
  **나중에 저장한 쪽**을 쓴다. `savedAt` 은 `defaultState()` 에 있어야 한다
  (bootUser 가 `k in S` 인 키만 복사하므로 빠지면 조용히 사라진다).
- 화면 오른쪽 위 표시등: `📴` 오프라인 · `⚠️` 서버 끊김 · `☁️` 정상.

## 오락실 (2026-09-11 신설)

편의점 안의 짧은 게임. 1판 `ARCADE_FEE`(100원), `ARCADE_SEC`(60초), 정답 1개 `ARCADE_HIT`(10원),
**하루 상한 `ARCADE_CAP`(300원)** + **게임마다 하루 `ARCADE_PLAYS`(5판)** — 공부 대신 게임으로
돈을 버는 걸 막는 장치라 없애지 말 것. 판 수는 `S.arcDay.plays[게임]` 에 센다
(옛 기록에 `plays` 칸이 없을 수 있어 `arcToday()` 가 매번 채워 준다).

**난이도 조절은 맨 위 상수 네 개만 고친다** — `ARCADE_SEC` · `MOLE_WAIT`(두더지가 기다리는 시간)
· `BALLOON_RISE`(풍선이 올라가는 시간) · `ARCADE_PLAYS`.

> 2026-09-11 대표 지시로 속도를 **절반**으로 늦췄다(두더지 2.6초→5.2초, 풍선 5.4초→10.8초).
> 이때 **게임 길이도 30초→60초로 같이 늘렸다.** 속도만 늦추면 한 판에 나오는 문제 수가 반으로 줄어
> 입장료 100원을 못 건지는 게임이 된다. **앞으로 속도를 바꿀 때는 길이도 같은 비율로 바꿀 것.**

- 게임 둘: `mole`(한글 뜻 → 영어 단어 두더지) · `balloon`(영어 단어 → 뜻 풍선).
- 문제 단어는 `arcPool()` — **배운 단어 우선**, 8개가 안 되면 지금 레벨 단어로 채운다.
- 모든 시계는 `ARC` 안에 있고 `arcStop()` 이 멈춘다. **`go()` 첫머리에서 반드시 부른다** —
  안 그러면 다른 화면으로 가도 게임이 백그라운드에서 계속 돌며 점수를 깎는다.
- 하루 기록 `S.arcDay`, 최고 점수 `S.arcBest`.

## 무르기(환불) (2026-09-11 신설)

기록 탭 영수증에서 `[↩︎ 무르기]` → `S.money` 만 되돌리고 `S.buys` 에서 뺀다.
살 때 `S.earned` 는 안 건드렸으므로 무를 때도 건드리지 않는다.

- **뽑기 상품(`id==="claw"`)은 무르지 못한다.** 무르고 다시 뽑기를 반복하면 원하는 상품이 나올 때까지
  돌릴 수 있어 뽑기가 무의미해진다.
- **[✅ 받았어요] 표시된 것도 못 무른다** — 부모가 이미 실제로 사 준 물건이다.
- 두 경우 모두 단추 자체를 안 보여주고, 함수(`doRefund`)에서도 한 번 더 막는다.
...
**상점은 계정마다 다르다.** 부모가 [우리 집 상점 꾸미기]에서 저장하면
`S.items` 에 들어가고, 기존 `save`/`load` 를 타고 서버(구글시트 '상태')에 함께 저장된다.
**Code.gs 는 손대지 않았다** — 새 액션이 필요 없다.

- 저장 형태는 `[이모지, 이름, 가격, 분류]` 네 칸짜리 줄(`packItems`/`unpackItems`).
  객체로 넣으면 상태 JSON 이 커져 시트 한 칸 한도(5만 자)에 가까워지므로 줄여서 넣는다.
  예전에 객체로 저장된 것도 `unpackItems` 가 그대로 받아들인다.
- `ITEMS_FAMILY` 가 true 면 **공용 상점을 덮어쓰지 않는다**(`loadItemsCache`/`loadItemsServer` 가 바로 반환).
- 우리 집 상점이 없는 계정은 예전처럼 서버 '상점' 탭 → 내장 기본값 순으로 쓴다.
- 대표(서버 `ADMIN_IDS` 계정)에게만 [☁️ 모든 집의 기본 상점으로도 저장] 단추가 더 보인다.

### 부모 모드 (2026-08-28)

상점·레벨·복습상한·레벨테스트·데이터초기화는 **부모 모드에서만** 보인다.
설정 → [🔒 부모 모드] → 잠금번호 4자리(`S.ppin`). 통과하면 `PARENT=true`.
대표처럼 서버 관리자 계정(`SESSION.admin`)이면 번호 없이 항상 열린다(`isParent()`).
`PARENT` 는 저장하지 않으므로 앱을 새로 열면 다시 잠긴다.
잠금번호는 상태에 그대로 들어 있어 브라우저 개발자도구로는 보인다 — 형제자매 장난을 막는 수준의 잠금이다.

- 관리자 화면(`scr-admin`, `renderAdmin`)에서 이모지/이름/가격/분류를 편집.
...
- **월드 0 「병아리 들판 🐥」**(=레벨 0)이 2026-08-28 에 앞에 붙어 총 6개 월드가 됐다.
  스테이지 번호가 8칸 밀리므로 `QS.v` 를 2로 올리고 부팅 때 한 번만 `startStage += stagesInWorld(0)` 로 보정한다.
  `cardATK` 는 `Math.max(lv,1)*10` 이라 기존 카드의 공격력은 그대로다.
- 구조: 6개 월드(=레벨 0~5) × 8스테이지(단어 10개, 마지막은 5개) = 40스테이지.
